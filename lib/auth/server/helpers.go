...
func NewTestAuthServer(cfg TestAuthServerConfig) (*TestAuthServer, error) {
	...
	srv.AuthServer, err = New(&InitConfig{
		Backend:                srv.Backend,
	...
	if err = srv.AuthServer.Services.Trust.UpsertCertAuthority(test.NewTestCAWithConfig(test.TestCAConfig{
		Type:        types.HostCA,
	...
	srv.Authorizer, err = NewAuthorizer(srv.AuthServer.Services.Access, srv.AuthServer.Services.Identity, srv.AuthServer.Services.Trust)
	if err != nil {
		return nil, trace.Wrap(err)
	}
	return srv, nil
...
func (a *TestAuthServer) GenerateUserCert(key []byte, username string, ttl time.Duration, compatibility string) ([]byte, error) {
	...
	checker, err := auth.FetchRoles(user.GetRoles(), a.AuthServer, user.GetTraits())
	if err != nil {
		return nil, trace.Wrap(err)
	}
...
func generateCertificate(authServer *Server, identity TestIdentity) ([]byte, []byte, error) {
	...
	case LocalUser:
		...
		checker, err := auth.FetchRoles(user.GetRoles(), authServer, user.GetTraits())
		if err != nil {
			return nil, nil, trace.Wrap(err)
		}
		if identity.TTL == 0 {
...
// FiXME(dmitri): move to lib/auth/client
/*
