...
func (a *Server) GenerateHostCert(hostPublicKey []byte, hostID, nodeName string, principals []string, clusterName string, roles teleport.Roles, ttl time.Duration) ([]byte, error) {
	...
	ca, err := a.Services.ServerTrust.GetCertAuthority(types.CertAuthID{
		Type:       types.HostCA,
...
func (a *Server) GetTokens(opts ...services.MarshalOption) (tokens []types.ProvisionToken, err error) {
	// get node tokens:
...
func (a *Server) DeleteRole(ctx context.Context, name string) error {
	...
	cas, err := a.Services.ServerTrust.GetCertAuthorities(types.UserCA, false)
	if err != nil {
		return trace.Wrap(err)
	}
	for _, a := range cas {
...
func (a *Server) GetNodes(namespace string, opts ...services.MarshalOption) ([]types.Server, error) {
