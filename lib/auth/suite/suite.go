...
type ServicesTestSuite struct {
	...
	ProvisioningS   local.Provisioner
	WebS            local.Identity
	ConfigS         local.ClusterConfiguration
...
// used in tests
type NewProxyWatcherFunc func() (*auth.ProxyWatcher, error)

func (s *ServicesTestSuite) Users() auth.UsersService {
	if s.WebS != nil {
		return s.WebS
	}
	return s.UsersS
...
func (s *ServicesTestSuite) CertAuthCRUD(c *check.C) {
	...
	cas, err = s.CAS.GetCertAuthorities(types.UserCA, true, resource.SkipValidation())
	c.Assert(err, check.IsNil)
	fixtures.DeepCompare(c, cas[0], ca)
